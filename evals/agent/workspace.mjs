...
/** Stage the same complete payload as the installers; caller owns the returned directory. */
export function stageSkillBundle(repoRoot, stagingParent = os.tmpdir()) {
  const dir = mkdtempSync(path.join(stagingParent, 'bp-skill-'));
  let complete = false;
  try {
    for (const part of ['SKILL.md', 'references', 'scripts', 'assets']) {
      cpSync(path.join(repoRoot, part), path.join(dir, part), { recursive: true });
    }
    complete = true;
    return dir;
  } finally {
    if (!complete) rmSync(dir, { recursive: true, force: true });
  }
}
...
export function buildPrompt({ arm, armFile }) {
  ...
  const core = [
    ...
    `Keep deliverable code inside ${outside}; task records may live in \`.ai/\`. Do not modify seeded files.`,
