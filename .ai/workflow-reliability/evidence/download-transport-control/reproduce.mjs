...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/control.csv') {
    res.writeHead(200, { 'Content-Type': 'text/csv; charset=utf-8',
      'Content-Disposition': 'attachment; filename="control.csv"', 'Content-Length': csv.length });
    res.end(csv);
  } else if (req.url === '/') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    res.end('<!doctype html><title>Download transport control</title><a id="download" href="/control.csv" download>Download control CSV</a>');
  } else {
    res.writeHead(204);
    res.end();
  }
...
try {
  ...
  assert.deepEqual(await readFile(join(downloads, 'control.csv')), csv);
  report.checks.push({ name: 'Managed-launch ordinary click saves exact CSV bytes', status: 'passed',
    bytes: csv.length, sha256: createHash('sha256').update(csv).digest('hex') });
