"""Bounded correction evidence; protocol fixtures are not real quality proof."""
...
def census():
    ...
    paths.add(HERE / "c1-code-review.md")
...
def main():
    label = sys.argv[1]
    pattern = {"red": "test_workflow_gate.py", "workflow": "test_workflow_*.py",
               "evidence": "test_evidence.py"}[label]
    base = "c1-correction-r3-" + label
    ...
    if any(p.exists() for p in outputs.values()):
        raise FileExistsError("Preserve earlier capture")
