...
| AC04 | P0 | Real guarded command admission with explicit trust limits | C2a preserved; C2b-S accepted; ordinary CLI not implemented | `evidence/c2b-state-independent-r2-report.md`: 110 distinct passes. Separate `evidence/c2b-state-code-review.md`: APPROVE for the two validation seams only. Five-verb CLI publication/admission remains due; positive recovery blocked. |
...
| AC09 | All | Independent per-increment verification/review and preserved regressions | C1/Q1 and C2a preserved; C2b-S approved; quality closure BLOCKED | C1/Q1 historical121-method proof in `5dabb4c`; C2a51 methods in `d971634`; C2b-S110 distinct passes and separate approval. CLI tests: zero. Q2 CHECK-TOOLS qualified, not adapter/metric acceptance. |
...
gates and were preserved in `5dabb4c`; C2a is preserved in `d971634`. C2b-S state seams are independently verified/reviewed for preservation; CLI proof and I1.T4/Q still block release.
